...
class BookingForm(forms.ModelForm):
    ...
    start_datetime = forms.DateTimeField(
        ...
        widget=forms.DateInput(
            attrs={
                "type": "date",
                "class": "w-full rounded-2xl border border-white/40 bg-white/10 px-4 py-3 text-white placeholder-white/60 backdrop-blur",
            }
        ),
    )
    ...
    def clean(self):
        ...
        if start:
            start = self._normalise_boundary(start, is_start=True)
            cleaned_data["start_datetime"] = start
        if end:
            end = self._normalise_boundary(end, is_start=False)
    ...
    def _normalise_boundary(self, value, *, is_start: bool) -> datetime:
        if not isinstance(value, datetime):
            if hasattr(value, "year") and hasattr(value, "month") and hasattr(value, "day"):
                value = datetime.combine(value, time.min if is_start else time.max)
            else:
                return value
        base = datetime.combine(
            value.date(),
            time.min if is_start else time(hour=23, minute=59, second=59),
        )
        current_timezone = timezone.get_current_timezone()
        if timezone.is_naive(base):
            return timezone.make_aware(base, current_timezone)
        return base.astimezone(current_timezone)
